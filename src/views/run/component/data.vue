...
<script>
import { run_LTable, run_search, run_coke_delete } from '@/api/run'
...
export default {
  ...
  methods: {
    ...
    async tableList() {
      const res = await run_LTable(this.page)
      console.log(res)
    ...
    async run_table_select() {
      try {
        const res = await run_search(this.manageform)
        this.$message({
          message: '搜索成功',
          type: 'success'
        })
    ...
    async delect(id) {
      try {
        const ls = await run_coke_delete(id)
        if (ls > 0) {
          this.$message({
            message: '删除成功',
            type: 'success'
